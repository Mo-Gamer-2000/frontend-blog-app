...
const SuggestedPosts = ({ className, header, posts = [], tags }) => {
    return (
        <div className={`w-full shadow-[rgba(7,_65,_210,_0.1)_0px_9px_30px] rounded-lg p-4 ${className}`}>
                ...
                {posts.map((item) => (
                    <div key={item._id} className="flex space-x-3 flex-nowrap items-center">
                        <img className="aspect-square object-cover rounded-lg w-1/5"
                            src={item?.image ? stables.UPLOAD_FOLDER_BASE_URL + item?.image : images.samplePostImage} alt={item.title} />
            ...
            {tags.length === 0 ? <p className="text-slate-500 text-xs mt-2">There is no tags for this post</p> : <div className="flex flex-wrap gap-x-2 gap-y-2 mt-4">
                {tags.map((item, index) => (
                    <Link key={index} to="/"
